document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!continentName) {
        ...
        document.body.innerHTML = "<h1>Configuration Error</h1><p>This admin page is not linked to a continent.</p>";
        return;
    }
    ...
    const dashboardTitle = document.querySelector('.nav__logo');

    const formattedName = continentName.charAt(0).toUpperCase() + continentName.slice(1).replace('-', ' ');
    dashboardTitle.innerHTML = `<i class="ri-settings-3-line"></i> ${formattedName} Admin`;
    ...
    const API_BASE = `/api/${continentName}`;
